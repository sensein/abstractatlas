...
- [ ] T014 [US1] Implement `src/ohbm2026/atlas_hosting/uploader.py` happy path to pass T011: discover artifacts in `--package-dir` (required `ohbm2026`/`neuroscape`/`atlas` + optional `neuroscape_vectors`); read each parquet's `build_info.state_key` (best-effort) for `source_build_state_key`; for each artifact hash → key → `object_exists` → upload-if-absent / skip-if-present; assemble `ContentAddressedObject`s, write the manifest, and build the `channel_entry`. Depends on T007, T009, T013.
- [ ] T015 [US1] Implement `src/ohbm2026/atlas_hosting/cli.py` `build_parser()` + `main()` for `upload-atlas-package` (args per `contracts/cli-upload-atlas-package.md`: `--package-dir` required, `--env-file` default `.env`, `--manifest-out` default `data/provenance/`, `--dry-run`), print the `channel_entry` JSON + summary to stdout; then register + dispatch it in `src/ohbm2026/cli.py` (mirror the `build-atlas-package` `_copy_actions` + dispatch pattern at `cli.py:58-62,255`).
...
- [ ] T024 [US2] Extend `src/ohbm2026/atlas_hosting/uploader.py` (and `cli.py` `--dry-run` wiring) to pass T018–T023: explicit skip-if-present branch with size guard (`ContentHashMismatchError`), `ArtifactDiscoveryError` for missing/unexpected artifacts, optional-vectors handling, and the `--dry-run` no-write path. Logs each skip explicitly (no silent no-op). Depends on T014, T015.
