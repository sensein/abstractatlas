...
- **SC-001**: A fresh Stage 4 run against the current Stage 3 bundles (corpus_state_key `f0c51e80dc0e`) produces all default-matrix annotation bundles in under **30 minutes** wall-clock on a single workstation. The default matrix is **32 bundles**: 10 `projections` (5 models × 2 inputs) + 10 `communities` (5 × 2) + 10 `topic_clusters` (5 × 2) + 2 `neuroscape_clusters` (`neuroscape` domain embedding × 2 inputs; `voyage`, `minilm`, `openai`, and `pubmedbert` are auto-skipped per FR-002's model-compatibility constraint).
