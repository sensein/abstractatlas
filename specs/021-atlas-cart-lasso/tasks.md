...
- [ ] T012 [US1] Rename the OHBM toggle in `site/src/routes/+page.svelte` (~:2338–2353): label "Saved only"→"Cart only", active "✓ Saved"→"✓ Cart", update `title`; keep `data-testid="toggle-cart-only"`; update the OHBM-home-only gating comment.
- [ ] T013 [US1] Add a "Cart only" toggle to atlas-root + neuroscape (in `site/src/lib/components/AtlasRootFacets.svelte` and `site/src/lib/components/NeuroscapeFacets.svelte`, or the atlas home header in `+page.svelte`), wired to the per-mode state, `data-testid="toggle-cart-only"`, disabled when the cart is empty and the filter is off.
- [ ] T014 [P] [US1] Render the cross-site warning (W1–W5) and empty states (E1–E3) in `site/src/lib/components/NeuroscapeBrowsePanel.svelte` and `site/src/lib/components/AtlasRootBrowsePanel.svelte`, driven by `hiddenCount`/`hiddenKinds` props passed from `+page.svelte`.
