...
class AtlasClusterCentroidsTests(unittest.TestCase):
    """Spec 019 — atlas.parquet must carry the cluster_centroids table so
    the cross-conference search bar can route an embedded query to a
    cluster. Without it atlas-root's semantic lane has no centroids to
    score against and silently returns zero NeuroScape hits."""
    ...
    def setUp(self) -> None:
        ...
        self.articles = _articles()
        self.clusters = _clusters()
        self.cluster_counts = {0: 3, 1: 2, 2: 1}
        self.palette = palette_mod.assign_palette(self.cluster_counts, primary_size=32)
        self.ohbm_overlay = _ohbm_overlay(n=4)
        self.ohbm_poster_ids = {
            sid: 200 + i for i, sid in enumerate(self.ohbm_overlay.submission_ids)
        }
        self.ohbm_titles = {sid: f"OHBM Title {sid}" for sid in self.ohbm_overlay.submission_ids}
        self.ohbm_nearest_cluster = {sid: 0 for sid in self.ohbm_overlay.submission_ids}
        self.centroids = {
            0: _embedded(1, 384, seed=10)[0],
            1: _embedded(1, 384, seed=11)[0],
            2: _embedded(1, 384, seed=12)[0],
        }

    def _write(self, *, cluster_centroids) -> None:
        parquet_writer.write_atlas_parquet(
            out_path=self.out,
            build_info=_build_info(state_key="atl000000001"),
            sibling_state_keys={"ohbm2026": "ohbm00000001", "neuroscape": "ns0000000001"},
            ...
            cluster_counts=self.cluster_counts,
            palette=self.palette,
            embedded_3d=_embedded(len(self.articles), 3, seed=1),
            embedded_2d=_embedded(len(self.articles), 2, seed=2),
            decimated_indices=np.array([0, 2, 3, 5], dtype=np.int64),
            ohbm_overlay=self.ohbm_overlay,
            ohbm_overlay_2d=_embedded(len(self.ohbm_overlay.submission_ids), 2, seed=99),
            ohbm_poster_ids=self.ohbm_poster_ids,
            ohbm_titles=self.ohbm_titles,
            ohbm_nearest_cluster=self.ohbm_nearest_cluster,
    ...
    def test_centroids_table_present_when_supplied(self) -> None:
        self._write(cluster_centroids=self.centroids)
        names = [n for n, _ in _decoded_outer_rows(self.out)]
        self.assertIn("cluster_centroids", names)

    def test_centroids_table_shape_matches_neuroscape_schema(self) -> None:
        self._write(cluster_centroids=self.centroids)
        outer = dict(_decoded_outer_rows(self.out))
        cc = _read_inner_table(outer["cluster_centroids"])
        self.assertEqual(
            set(cc.column_names), {"cluster_id", "centroid_vector", "member_count"}
        )
        self.assertEqual(cc.column("cluster_id").to_pylist(), [0, 1, 2])
        self.assertEqual(cc.column("member_count").to_pylist(), [3, 2, 1])
        self.assertEqual(len(cc.column("centroid_vector").to_pylist()[0]), 384)

    def test_centroids_table_omitted_when_absent(self) -> None:
        self._write(cluster_centroids=None)
        names = [n for n, _ in _decoded_outer_rows(self.out)]
        self.assertNotIn("cluster_centroids", names)
