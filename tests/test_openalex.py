...
class OpenAlexHelpersTest(unittest.TestCase):
    ...
    def test_extract_reference_entries_uses_llm_split_for_concatenated_block(self) -> None:
        ...
        with patch(
            "abstractatlas.enrich.openalex.llm_reference_split_request",
            return_value=[
                {
    ...
    def test_promote_openalex_doi_lifts_doi_to_top_level(self) -> None:
        ...
        from abstractatlas.enrich.openalex import _promote_openalex_doi
        reference = {
            "doi": None,
            "match_method": "title",
            "openalex": {
    ...
    def test_openalex_request_appends_api_key_from_environment(self) -> None:
        ...
        with patch("abstractatlas.enrich.openalex.urlopen_with_retries", side_effect=fake_urlopen):
    ...
    def test_openai_reference_split_request_uses_responses_api(self) -> None:
        ...
        with (
            patch("abstractatlas.enrich.openalex.get_openai_api_key", return_value="test-key"),
            patch("abstractatlas.enrich.openalex.urlopen_with_retries", side_effect=fake_urlopen),
        ):
    ...
    def test_search_semantic_scholar_doi_by_title_returns_best_matching_doi(self) -> None:
        ...
        with patch("abstractatlas.enrich.openalex.semantic_scholar_request", return_value=payload):
    ...
    def test_search_crossref_doi_by_title_returns_best_matching_doi(self) -> None:
        ...
        with patch("abstractatlas.enrich.openalex.crossref_request", return_value=payload):
    ...
    def test_resolve_reference_cache_doi_discovery_finds_doi_and_matches_openalex(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            output_path = Path(temp_dir) / "reference_metadata.json"
            with (
                patch("abstractatlas.enrich.openalex.search_semantic_scholar_doi_by_reference", return_value=("10.1/example", 0.93)),
                patch("abstractatlas.enrich.openalex.fetch_openalex_work_by_doi", return_value=openalex_work),
            ):
    ...
    def test_resolve_reference_cache_doi_discovery_does_not_fallback_to_crossref(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            output_path = Path(temp_dir) / "reference_metadata.json"
            with (
                patch("abstractatlas.enrich.openalex.search_semantic_scholar_doi_by_reference", return_value=(None, 0.42)),
                patch("abstractatlas.enrich.openalex.search_crossref_doi_by_title", side_effect=AssertionError("Crossref should not be called")),
            ):
                stats = resolve_reference_cache_doi_discovery(
                    abstract_reference_records,
                    reference_cache,
    ...
    def test_resolve_reference_cache_doi_discovery_skips_references_with_titles(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            output_path = Path(temp_dir) / "reference_metadata.json"
            with patch(
                "abstractatlas.enrich.openalex.search_semantic_scholar_doi_by_reference",
    ...
    def test_repair_failed_reference_splits_reruns_only_failed_abstracts(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            output_path = Path(temp_dir) / "reference_metadata.json"
            with patch(
                "abstractatlas.enrich.openalex.build_reference_metadata_database",
